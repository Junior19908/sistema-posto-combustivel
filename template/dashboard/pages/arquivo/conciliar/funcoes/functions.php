...
$QUERYCONT = mysqli_query($conn, "SELECT COUNT(*) AS CONT FROM tb_import WHERE date BETWEEN '$inicio' AND '$fim' AND funcionario_cod_veiculo IS NOT NULL");
...
$QUERYEXCEL = mysqli_query($conn, "SELECT * FROM tb_export WHERE mes ='$MES' AND safra = '$SAFRA'");
$ROWCONT = mysqli_num_rows($QUERYEXCEL);
if ($ROWCONT == 0){
  
  $QUERYCONT = mysqli_query($conn, "SELECT IMP.*, FUNC.*, SUM(litros) AS SOMALITROS, SUM(valor) AS SOMAVALOR FROM tb_import AS IMP LEFT JOIN tb_funcionario AS FUNC ON imp.funcionario_cod_veiculo = FUNC.cod_veiculo WHERE IMP.date BETWEEN '$inicio' AND '$fim' AND IMP.funcionario_cod_veiculo IS NOT NULL GROUP BY IMP.funcionario_cod_veiculo");
...
$VEICULO = $row["funcionario_cod_veiculo"];
...
$VALORD = $row["SOMAVALOR"];
  ...
  echo "</tbody>
    ...
    $mysql = "INSERT INTO `tb_export` (`id_export`, `funcionario`, `litros`, `valor`, `date`,`now_date`, `id_user`, `mes`, `safra`) VALUES (NULL, '$VEICULO', '$LITROSD', '$VALORD', '$inicio', NOW(), '$ID_USER', '$MES', '$SAFRA')";
    $insert = mysqli_query($conn,$mysql);
    }
    $mysql = "INSERT INTO `tb_tarefas` (`id_tarefa`, `titulo`, `marqCheck`, `data`, `date_now`, `hora`, `id_user`) VALUES (NULL, 'Conciliação feita, arquivo excel pronto.', NULL, CURDATE(), CURDATE(), NOW(), '$ID_USER')";
    $insert = mysqli_query($conn,$mysql);
  }else{
