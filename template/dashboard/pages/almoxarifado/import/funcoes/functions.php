...
function inverteData($DATASD){
	if(count(explode("/",$DATASD)) > 1){
		return implode("-",array_reverse(explode("/",$DATASD)));
	}elseif(count(explode("-",$DATASD)) > 1){
		return implode("/",array_reverse(explode("-",$DATASD)));
	}
  }
...
$ModDat = array(".");
$ModDatBarr = array("-");
$ModDatBarrStr = str_replace($ModDat, $ModDatBarr, $junt_str);


foreach($ModDatBarrStr as $linha){
$linha = trim($linha);
$valor = explode('|', $linha);
...
$DATAINVERTIDA = inverteData($DATASD);
