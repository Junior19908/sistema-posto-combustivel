...
WHERE col_posicao=40076 AND tb_relatorioentradafornecedor.col_materialCodigoSAP NOT BETWEEN 270000 AND 279999
...
if ($CONSULTAQUERY > 0) {
    ...
    FROM `tb_relatorioentradafornecedor` WHERE col_posicao=40076");
    ...
    FROM `tb_relatorioentradafornecedor` WHERE col_posicao=40076 AND col_materialCodigoSAP NOT BETWEEN 270000 AND 279999");
    $row = mysqli_num_rows($sql);
    if ($row > 0){
            while($sqline = mysqli_fetch_array($sql)){
                    $v40076 = $sqline["SOMAVALORMATERIAL"];
            }
    }
    $pdf->SetX(265);
    $pdf->Cell(22,4,"R$: ".number_format($v40076, 2, ",", "."),1,2,"R");
    $pdf->Ln();
}
