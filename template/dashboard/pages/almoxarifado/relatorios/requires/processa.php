<?php
for ($i=40000; $i <= 40500; $i++) { 

//$posicao = $_POST["posicao"];
$posicao = $i;
$arquivo = fopen($posicao.".php","w");
$escrita = '
<?php
...
WHERE col_posicao='.$posicao.' AND tb_relatorioentradafornecedor.col_materialCodigoSAP NOT BETWEEN 270000 AND 279999
...
if ($CONSULTAQUERY > 0) {
    ...
    FROM `tb_relatorioentradafornecedor` WHERE col_posicao='.$posicao.'");
    ...
    FROM `tb_relatorioentradafornecedor` WHERE col_posicao='.$posicao.' AND col_materialCodigoSAP NOT BETWEEN 270000 AND 279999");
    $row = mysqli_num_rows($sql);
    if ($row > 0){
            while($sqline = mysqli_fetch_array($sql)){
                    $v'.$posicao.' = $sqline["SOMAVALORMATERIAL"];
            }
    }
    $pdf->SetX(265);
    $pdf->Cell(22,4,"R$: ".number_format($v'.$posicao.', 2, ",", "."),1,2,"R");
    $pdf->Ln();
}
';
