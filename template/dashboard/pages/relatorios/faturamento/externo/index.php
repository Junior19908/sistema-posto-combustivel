...
<head>
  ...
  <title>SistemaGSG | Mercado Externo</title>
...
<?php  
    // abre a conexão
    $PDO = ConexaoDBFaturamentos();
    // SQL para selecionar os registros
    $SQL_SELECT = "SELECT * FROM tb_saida_semana WHERE ESTADO='EX' ORDER BY DATA_EMISS DESC";
...
?>
  <div class="card">
    <div class="card-header">
      <h3 class="card-title">Mercado Externo</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
      <table id="example1" class="table table-bordered table-striped">
        ...
        while ($SQL = $resultado_conexao->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
                    <td><center><?php echo $SQL['CIDADE']; ?></center></td>
                    <td><center><?php echo $SQL['ESTADO']; ?></center></td>
                    <td><center><?php echo $SQL['DESCRICAO']; ?></center></td>
                    <td><center><?php echo $SQL['COD_RECEB']; ?></center></td>
                    <td><center><?php echo date("d/m/Y",strtotime($SQL['DATA_EMISS'])); ?></center></td>
                    <td>
                      <center>
                        <button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#exampleModal" 
                        data-idbmb="<?php echo $SQL['ID_SAIDA']; ?>"
                        data-bomba1="<?php echo $SQL['CIDADE']; ?>"
                        data-bomba2="<?php echo $SQL['ESTADO']; ?>"
                        data-bomba3="<?php echo $SQL['DESCRICAO']; ?>"
                        data-bomba4="<?php echo $SQL['COD_RECEB']; ?>"
                        data-databomba="<?php echo date("d/m/Y",strtotime($SQL['DATA_EMISS'])); ?>"
                        >Alterar</button>
                      </center>
                    </td>
                </tr>
      ...
      <div class="modal-body">
        <form role="form" name="formCad" action="" method="post">
          <div class="form-group">
              <label for="ESTADOVL" class="col-form-label">ESTADO:</label>
              <input name="ESTADOVL"  type="text" class="form-control" id="ESTADOVL">
          </div>
        </div>
        <input name="ID_SAIDAEST" hidden id="ID_SAIDAEST">
        <div class="modal-footer justify-content-between">
          <button type="button" class="btn btn-outline-light" data-dismiss="modal">Cancelar</button>
          <button type="submit" name="btnUpdateEstado" class="btn btn-outline-light">Alterar</button>
        </div>
      </form>
    </div>
    <!-- /.modal-content -->
...
<script type="text/javascript">
  $('#exampleModal').on('show.bs.modal', function (event) {
    ...
    var recipientbmb1 = button.data('bomba2') // Extrai informação dos atributos data-*
    var recipientID = button.data('idbmb')
    // Se necessário, você pode iniciar uma requisição AJAX aqui e, então, fazer a atualização em um callback.
    // Atualiza o conteúdo do modal. Nós vamos usar jQuery, aqui. No entanto, você poderia usar uma biblioteca de data binding ou outros métodos.
    var modal = $(this)
    modal.find('.modal-title').text('Id Nº: ' + recipientID)
    modal.find('#ID_SAIDAEST').val(recipientID)
    modal.find('#ESTADOVL').val(recipientbmb1)
  })
</script>
    </body>
</html>