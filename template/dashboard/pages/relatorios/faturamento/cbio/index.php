...
<head>
  ...
  <title>SistemaGSG | Notas Fiscais CBIO</title>
          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Notas Fiscais CBIO</li>
              </ol>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid RelatorioCheckList.php RelatorioAcesso.php-->
      </div>
    <!-- Div -->
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Lista de Notas Fiscais para CBIO - Consultada no Site do Porta da Nota Fiscal | www.nfe.fazenda.gov.br | <?php echo($CONCLUIDA); ?> <- CONCLUÍDAS | <a href="RelatorioCbio.php"><?php echo($PENDENTES); ?> <- PENDENTE DE CONFIRMAÇÃO | </a> <?php echo($SMANIFEST); ?> <- SEM MANIFESTAÇÃO</h3>
          </div>
          <!-- /.card-header -->
            ...
            <table class="table table-condensed">
              <thead>
                <tr>
                  ...
                  <th style="text-align: center;">Data da NFe</th>
                  <th style="text-align: center;width: 30%;">-</th>
                    ...
                    <?php  
                        ...
                        $SQL_SELECT = "SELECT * FROM `tb_nota_consultar` ORDER BY col_dataNfe DESC, col_Nfe DESC;";
              ...
              while ($SQL = $resultado_conexao->fetch(PDO::FETCH_ASSOC)): ?>
                  <?php
                      $NFE = $SQL['col_Nfe'];
                      $STATUSNFE = $SQL['col_status'];

                      if ($STATUSNFE==1) {
                          $PORCENTAGEM = 15;
                          $CLASS = "danger";
                          $STATUS="Não Verificado!";
                          $CONTAGEMORDEM="Aguadando o Administrador, Baixar Informações.";
                      }elseif($STATUSNFE==2){
                          $PORCENTAGEM = 100; 
                          $CLASS = "success";
                          $STATUS="Concluído!";
                          $CONTAGEMORDEM="Confirmação da Operação pelo Destinatário (Órgão Autor: AN)";
                      }elseif($STATUSNFE==142){
                          $PORCENTAGEM = 0; 
                          $CLASS = "primary";
                          $STATUS="Não Manifestado!";
                          $CONTAGEMORDEM="Aguadando.....!";
                      }else{
                          $PORCENTAGEM = 50;
                          $CLASS = "warning";
                          $STATUS="Parcialmente!";
                          $CONTAGEMORDEM="Ciência da Operação pelo Destinatário (Órgão Autor: AN)";
                      }
                  ?>
                      <tr>
                          <td style="text-align: center;width:10%;"><?php echo $SQL['col_chaveAcesso']; ?></td>
                          <td style="text-align: center;width:25%;"><?php echo $SQL['col_nomeCliente']; ?></td>
                          <td style="text-align: center;width:1%;"><?php echo $SQL['col_Nfe']; ?></td>
                          <td style="text-align: right;width:10%;"><center><a tabindex="0" title="Evento da Nota Fiscal" data-toggle="popover" data-placement="top" data-content="<?php echo($CONTAGEMORDEM); ?>"><?php echo $STATUS; ?></a></center></td>
                          <td style="text-align: center;width:1%;"><?php echo date("d/m/Y",strtotime($SQL['col_dataNfe'])); ?></td>
                            <td>
                              <div class="progress progress-sm">
                                <div class="progress-bar bg-<?php echo($CLASS); ?>" style="width: <?php echo($PORCENTAGEM); ?>%"></div>
                              </div>
                            </td>
                            <td><span class="badge bg-<?php echo($CLASS); ?>"><?php echo($PORCENTAGEM); ?>%</span></td></tr>
                    <?php endwhile; ?>
              </tbody>
            </table>
          </div>
