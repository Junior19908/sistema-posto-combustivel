        ...
        if ($CONSULT > 0) {
            ...
            if($ROW1 > 0){
                while ($Linha = $QUERY1->fetch_array()){
                    ...
                    if ($Linha['col_un']=='KG') {
                        $QTDCONTRATO40056_SEMANA_QUITADAS = $Linha['col_qtd_contrato']/50;
                    }elseif($Linha['col_un']=='SC') {
                        $QTDCONTRATO40056_SEMANA_QUITADAS = $Linha['col_qtd_contrato'];
                    }elseif($Linha['col_un']=='TM'){
                        $QTDCONTRATO40056_SEMANA_QUITADAS = $Linha['col_qtd_contrato']*20;
                    }
                    $DATACONTRATO_SEMANA_QUITADAS = $Linha['col_data'];
                }
            }

            $SOMAQUANTIORDQUITADA = mysqli_query($CONNECTIONFATURAMENTO, "SELECT OV.*,CONTRATO.*, SUM(Qtd_conf) AS SOMASE FROM tb_ordem_venda AS OV LEFT JOIN tb_contratos AS CONTRATO ON ov.N_pedido=contrato.col_docvendas WHERE contrato.col_position='40056' AND CONTRATO.col_repres='9' AND OV.SAFRA='$SAFRA'");
            $COSULTORDQUIT = mysqli_num_rows($SOMAQUANTIORDQUITADA);
            if($COSULTORDQUIT = 1){
                while ($Linha = $SOMAQUANTIORDQUITADA->fetch_array()){
                    if ($row['UNIDADE']=='KG') {
                        $Q40056_SEMANA_QUITADAS = $Linha['SOMASE']/50;
                    }else{
                        $Q40056_SEMANA_QUITADAS = $Linha['SOMASE'];
                    }
                }
            }

            $S40056_SEMANA_QUITADAS =  $QTDCONTRATO40056_SEMANA_QUITADAS - $QTDCONTRATO40056KG_SEMANA_QUITADAS - $Q40056_SEMANA_QUITADAS;
            ...
            $pdf->Row(Array(
                ...
                ' ',
                number_format($QTDCONTRATO40056_SEMANA_QUITADAS, 0,',','.'),
                number_format($Q40056_SEMANA_QUITADAS, 0,',','.'),
                number_format($S40056_SEMANA_QUITADAS, 0,',','.')
            ));
            $pdf->SetX(10);
            $pdf->SetFont('Arial','',5);
	...
	while($row = $QUERY->fetch_array()){
        ...
        if($ROWOVQUITADA > 0){
            while ($Linha = $QUERYOVQUITADA->fetch_array()){
                if($row['UMB'] == 'KG'){
                    $QUERYORDQUITADA40056_SEMANA_QUITADAS = $Linha['Qtd_conf']/50;
                }else {
                    $QUERYORDQUITADA40056_SEMANA_QUITADAS = $Linha['Qtd_conf'];
                } 
            }
        }
		...
		$pdf->Row(Array(
            ...
            number_format($QUERYORDQUITADA40056_SEMANA_QUITADAS, 0,',','.'),
            number_format($OVREPQUIT_SEMANA_QUITADAS, 0,',','.'),
            number_format($QUERYORDQUITADA40056_SEMANA_QUITADAS - $OVREPQUIT_SEMANA_QUITADAS, 0,',','.'),
        ));
	}
    $pdf->Ln(2);
    }else{
    $QUERYMYSQL = mysqli_query($CONNECTIONFATURAMENTO, "SELECT * FROM tb_contratos WHERE col_position='40056' AND SAFRA='$SAFRA' AND col_repres='9'  ORDER BY `col_nome` ASC");
    $CONSULTMYSQL = mysqli_num_rows($QUERYMYSQL);

    if ($CONSULTMYSQL > 0) {
        $QUERY2 = mysqli_query($CONNECTIONFATURAMENTO, "SELECT * FROM `tb_contratos` WHERE col_position='40056' AND SAFRA='$SAFRA' AND col_repres='9'");
        ...
        $pdf->SetFont('Arial', 'BI', 6);
        $pdf->Cell(195, 3,utf8_decode('CONTRATO REFERENTE À - ') .number_format($QTDCONTRATO40056_SEMANA_QUITADAS, 0, ',', '.'). utf8_decode(' SC'), 1, 2, 'R',true);
        $pdf->SetX(10);
        $pdf->SetFont('Arial','',5);
        $pdf->Ln(2);
    }
