<?php
        ...
        $QUERY = mysqli_query($CONNECTIONFATURAMENTO, "SELECT CONTRATO.*, OV.*, SAIDA.* FROM tb_contratos AS CONTRATO LEFT JOIN tb_ordem_venda AS OV on CONTRATO.col_docvendas=OV.N_pedido LEFT JOIN tb_saida_semana AS SAIDA ON OV.Doc_SD=SAIDA.ORDEM WHERE OV.UM='1' AND CONTRATO.col_position='00009' AND SAIDA.SAFRA='$SAFRA' GROUP BY OV.Doc_SD ORDER BY `CONTRATO`.`col_nome` ASC");
        $CONSULT = mysqli_num_rows($QUERY);

        if ($CONSULT > 0) {
            $QUERY1 = mysqli_query($CONNECTIONFATURAMENTO, "SELECT * FROM `tb_contratos` WHERE col_position='00009' AND SAFRA='$SAFRA'");
            ...
            if($ROW1 > 0){
                while ($Linha = $QUERY1->fetch_array()){
                    ...
                    if ($Linha['col_un']=='KG') {
                        $QTDCONTRATO00009KG = $Linha['col_qtd_contrato']/50;
                    }else {
                        $QTDCONTRATO00009 = $Linha['col_qtd_contrato'];
                    }
                    $DATACONTRATO = $Linha['col_data'];
                }
            }

            $SOMAQUANTIORDQUITADA = mysqli_query($CONNECTIONFATURAMENTO, "SELECT OV.*,CONTRATO.*, SUM(Qtd_conf) AS SOMASE FROM tb_ordem_venda AS OV LEFT JOIN tb_contratos AS CONTRATO ON ov.N_pedido=contrato.col_docvendas WHERE contrato.col_position='00009' AND OV.UM='1' AND OV.SAFRA='$SAFRA' GROUP BY UMB");
            $COSULTORDQUIT = mysqli_num_rows($SOMAQUANTIORDQUITADA);
            if($COSULTORDQUIT = 1){
                while ($Linha = $SOMAQUANTIORDQUITADA->fetch_array()){
                    if ($Linha['UMB']=='KG') {
                        $Q00009KG = $Linha['SOMASE']/50;
                    }else{
                        $Q00009 = $Linha['SOMASE'];
                    }
                }
            ...
            $S00009 =  ($QTDCONTRATO00009KG + $QTDCONTRATO00009) - ($Q00009 + $Q00009KG);
            ...
            $pdf->Row(Array(
                ...
                '-',
                number_format($QTDCONTRATO00009+$QTDCONTRATO00009KG, 0,',','.'),
                number_format($Q00009+$Q00009KG, 0,',','.'),
                number_format($S00009, 0,',','.')
            ));
            $pdf->SetX(10);
            $pdf->SetFont('Arial','',5);
            $pdf->SetWidths(array(55, 36,18, 15, 17, 15.5, 20, 18.5));
    ...
    $QUERYMYSQL = mysqli_query($CONNECTIONFATURAMENTO, "SELECT * FROM tb_contratos WHERE col_position='00009' AND SAFRA='$SAFRA' ORDER BY `col_nome` ASC");
    $CONSULTMYSQL = mysqli_num_rows($QUERYMYSQL);

    if ($CONSULTMYSQL > 0) {
        $QUERY2 = mysqli_query($CONNECTIONFATURAMENTO, "SELECT * FROM `tb_contratos` WHERE col_position='00009' AND SAFRA='$SAFRA'");
        ...
        $pdf->SetFont('Arial', 'BI', 6);
        $pdf->Cell(195, 3,utf8_decode('CONTRATO REFERENTE À - ') .number_format($QTDCONTRATO00009, 0, ',', '.'). utf8_decode(' SC'), 1, 2, 'R',true);
        $pdf->SetX(10);
        $pdf->SetFont('Arial','',5);
