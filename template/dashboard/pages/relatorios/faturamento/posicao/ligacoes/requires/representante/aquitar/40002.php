<?php
        ...
        if ($CONSULT > 0) {
            ...
            if($ROW1 > 0){
                while ($Linha = $QUERY1->fetch_array()){
                    ...
                    if ($Linha['col_un']=='KG') {
                        $QTDCONTRATO40002KG_AQUITAR = $Linha['col_qtd_contrato']/50;
                    }else {
                        $QTDCONTRATO40002_AQUITAR = $Linha['col_qtd_contrato'];
                    }
            ...
            if($COSULTORDQUIT = 1){
                while ($Linha = $SOMAQUANTIORDQUITADA->fetch_array()){
                    if ($row['UNIDADE']=='KG') {
                        $Q40002_AQUITAR = $Linha['SOMASE']/50;
                    }else{
                        $Q40002_AQUITAR = $Linha['SOMASE'];
                    }
                }
            }

            $S40002_AQUITAR =  $QTDCONTRATO40002_AQUITAR - $QTDCONTRATO40002KG_AQUITAR - $Q40002_AQUITAR;
            ...
            $pdf->Row(Array(
                ...
                ' ',
                number_format($QTDCONTRATO40002_AQUITAR, 0,',','.'),
                number_format($Q40002_AQUITAR, 0,',','.'),
                number_format($S40002_AQUITAR, 0,',','.')
            ));
            $pdf->SetX(10);
            $pdf->SetFont('Arial','',5);
	...
	while($row = $QUERY->fetch_array()){
        ...
        if($ROWOVQUITADA > 0){
            while ($Linha = $QUERYOVQUITADA->fetch_array()){
                if($row['UMB'] == 'KG'){
                    $QUERYORDQUITADA40002_AQUITAR = $Linha['Qtd_conf']/50;
                }else {
                    $QUERYORDQUITADA40002_AQUITAR = $Linha['Qtd_conf'];
                } 
            }
        }
		...
		$pdf->Row(Array(
            ...
            number_format($QUERYORDQUITADA40002_AQUITAR, 0,',','.'),
            number_format($OVREPQUIT_AQUITAR, 0,',','.'),
            number_format($QUERYORDQUITADA40002_AQUITAR - $OVREPQUIT_AQUITAR, 0,',','.'),
        ));
	}
    $pdf->Ln(2);
    }else{
    ...
    if ($CONSULTMYSQL > 0) {
        ...
        $pdf->SetFont('Arial', 'BI', 6);
        $pdf->Cell(195, 3,utf8_decode('CONTRATO REFERENTE À - ') .number_format($QTDCONTRATO40002_AQUITAR, 0, ',', '.'). utf8_decode(' SC'), 1, 2, 'R',true);
        $pdf->SetX(10);
        $pdf->SetFont('Arial','',5);
        $pdf->Ln(2);
    }
