...
class PDF extends FPDF
{
  ...
  //***********************************************************//
      $this->SetFont('Arial','I',10);
      $this->Cell(-1);
      $this->Cell(30,12, utf8_decode('DEMONSTRATIVO DE COMBUSTÍVEL FORNECIDO AOS FORNECEDORES DE CANA'),0,0,'L');
      $this->Ln(12);
  //***********************************************************//
  }
}
...
$pdf->SetXY(80, 50);
$pdf->SetFont('Arial','',8);
$pdf->Cell(35,4,utf8_decode('FAZENDA'),1,2,'C');
$pdf->Ln();
$pdf->SetXY(115, 50);
$pdf->SetFont('Arial','',8);
...
if ($CONSULT > 0) {
	while($row = $QUERY->fetch_array()){
		$pdf->Row(Array(
            utf8_decode($row['nome_fornecedor']),
            utf8_decode($row['nome_fazenda']),
			$row['fornecedor_cod_veiculo'],
